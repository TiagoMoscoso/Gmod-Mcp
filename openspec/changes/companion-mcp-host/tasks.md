...
- [x] 2.3 Implement `get_agent` and `get_agent_status` keyed by `agent_id` and verify unknown ids return `unknown_agent`
- [x] 2.4 Assert the tool catalog has no per-character-name tools and verify a test fails if a `walter_*` tool is registered
...
- [x] 3.2 Track MCP client connected/disconnected and verify unit tests flip the flag on session start and end
- [x] 3.3 Expose `PROTOCOL_VERSION` from the Companion process (health or handshake field) and verify it matches the scaffold constant `"1"`
